...
**Burn-chance data:** cross-checked directly against individual Wiki item pages (Shark, Anglerfish, Monkfish, Swordfish, Lobster, Tuna, Salmon, Pike, Trout, Mackerel, Herring, Sardine) rather than trusted from one summarized table — an initial fetch of the Cooking/Burn_level page returned a shark burn-stop level that contradicted both the handoff's own required acceptance test and the Shark page itself, and turned out to be wrong on other rows too (e.g. Lobster's gauntlet stop). The exact burn-stop levels (the level at which success reaches exactly 100%) are verified; the sub-100% success curve between a food's unlock level and its burn-stop level uses a labeled linear-interpolation estimate (assumed 50% at unlock), since this session couldn't extract the game's exact low/high formula breakpoints — this is the handoff's own explicitly permitted fallback ("label it as a guide-table estimate" when the exact formula can't be verified). No acceptance test depends on an exact mid-level percentage; all of them check verified endpoints (Shark=100% at 94 with gauntlets, gauntlets not affecting karambwan, location attempts/hr, etc), which all pass exactly.
...
- **Manta ray has no verified sub-99 burn-stop level** (it's not on the Wiki's cooking-gauntlets list, unlike the other high-tier fish) — modeled as never fully stopping without a Cooking cape, and deliberately excluded from the curated Simple route (Anglerfish carries through to 99 instead) since the spec's own Simple progression doesn't mention it.
